...
1. [ ] Oben Sekundärbutton „Zurück zu Becken" → `/becken` (wie im Mockup).
2. [ ] Name als Überschrift (Display), darunter Metazeile (Volumen, Startdatum) mit den Formatierern aus TASK-04-04,
       darunter die Beschreibung, falls vorhanden.
3. [ ] Aktionen „Bearbeiten" (Link auf den Bearbeiten-Pfad) und Platz für „Löschen".
4. [ ] Zustände über `useTank(id)`: laden, Fehler, **nicht gefunden** („Becken nicht gefunden." und Link zur Beckenliste).

### B · Becken bearbeiten

5. [ ] Seite auf dem in TASK-04-01 festgelegten Pfad: Becken laden, dann `TankForm` mit den gespeicherten Werten
       vorbelegen. Überschrift „Becken bearbeiten".
6. [ ] Speichern ruft `updateTank`, danach zurück zum Beckendetail. „Abbrechen" führt ohne Speichern zum Beckendetail.
7. [ ] Lade-, Nicht-gefunden- und Fehlerzustand wie beim Detail; Speicherzustand wie in TASK-04-05.

## Fertig, wenn

- [ ] Eine Änderung ist nach dem Neuladen der Seite noch da
- [ ] Ein Feld leeren (z. B. Volumen) speichert `null`, die Metazeile zeigt den Wert nicht mehr
- [ ] Validierung und Feldfehler verhalten sich wie beim Anlegen (FR-6.6)
- [ ] Eine unbekannte ID in der Adresse zeigt „Becken nicht gefunden", keinen Absturz und keine leere Seite
- [ ] Angemeldet als Testnutzer B zeigt die Adresse von As Becken (`/becken/aaaaaaaa-0000-0000-0000-000000000001`)
      „Becken nicht gefunden" (FR-6.2)
- [ ] Beckendetail mit Bottom-Navigation („Becken" aktiv), Bearbeiten-Seite ohne
