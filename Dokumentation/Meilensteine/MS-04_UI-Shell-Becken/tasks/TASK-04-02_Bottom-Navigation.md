...
- [x] **Icons.** Das Mockup nutzt eigene SVGs; `components.json` setzt `lucide` als Icon-Bibliothek.
      ...
      → Die vier SVGs aus dem Mockup, in `src/components/NavIcons.tsx`. Lucide trifft vor allem das
      Beckensymbol nicht. Nicht unter `src/components/ui/`: dort liegt nur, was die shadcn-CLI anlegt.

## Schritte

1. [x] Komponente `BottomNav` (z. B. `src/components/BottomNav.tsx`) mit `<nav aria-label="Hauptnavigation">`
       und vier `NavLink`-Einträgen auf die Pfade aus TASK-04-01.
2. [x] Jeder Eintrag: Icon 20 px mit `aria-hidden="true"`, darunter Textlabel 12/16/500 (`text-caption font-medium`).
...
4. [x] **Position und Maße:** `fixed` am unteren Rand, Höhe 64, Fläche `bg-card` (`#141E2E`), Oberkante 1 px `border-border`.
       Die vier Einträge teilen sich die Breite gleichmäßig (`flex-1 min-w-0`), jeder Eintrag volle Höhe.
5. [x] **Breite am Desktop:** `#root` hat `max-w-md`, das wirkt auf `fixed`-Elemente nicht. Die Navigation selbst
       auf dieselbe Breite begrenzen und zentrieren (z. B. `inset-x-0 mx-auto max-w-md`).
6. [x] `BottomNav` in `AppLayout` aus TASK-04-01 einsetzen.
7. [x] Sichtbarer Fokusrahmen für Tastaturbedienung (`focus-visible`).

## Fertig, wenn

- [x] Vier Einträge mit Icon **und** Text, der aktive Eintrag ist farbig hervorgehoben (NFR-1.4)
- [x] Auf `/becken/:id` ist „Becken" aktiv, auf `/` nur „Bestand"
- [x] Jeder Eintrag ist mindestens 44 × 44 px groß (NFR-1.3); bei 360 px Breite kein Umbruch der Labels (NFR-1.6)
- [x] Die Navigation fehlt auf Anmeldung, Registrierung und den Formularseiten
- [x] Der letzte Inhalt einer langen Seite lässt sich über die Navigation hinaus scrollen und ist lesbar
- [x] Am Desktop ist die Navigation so breit wie die Inhaltsspalte
- [x] Mit Tab erreichbar, aktiver Eintrag hat `aria-current="page"` (setzt `NavLink` automatisch)
