...
- [x] **Dateiname.** Vorschlag `src/services/tank.ts` (englisch wie `profile.ts`, `auth.ts`). → Festlegen.
  → **Entschieden:** `src/services/tank.ts`.
- [x] **Löschsperre (FR-1.1).** `koralle.becken_id` ist `ON DELETE RESTRICT`; die Datenbank lehnt das Löschen eines
  ...
  → **Entschieden:** (a) – `deleteTank` bildet `23503` auf eine eigene Meldung ab.
...
4. [x] Leere optionale Felder als `null` speichern, nicht als leere Zeichenkette.

## Fertig, wenn

- [x] Kein Import von `@supabase/*` außerhalb von `src/services/` (die ESLint-Regel aus TASK-03-07 meldet nichts)
