...
- [ ] **Dateiname.** Vorschlag `src/services/coral.ts` (englisch wie `tank.ts`), Hook `useCorals.ts`.
      → Festlegen.
- [ ] **Beckenname in der Liste.** Die Bestandsliste soll zeigen, in welchem Becken eine Koralle steht; `koralle` hat
      nur `becken_id`.
...
1. [ ] **Typen** aus den generierten Datenbanktypen ableiten, nicht handschreiben (NFR-4.4):
   ...
   - bei Entscheidung (a): Listentyp mit dem mitgeladenen Beckennamen
...
3. [ ] **Leere optionale Felder** (`art`, `handelsname`, `erwerbsdatum`) als `null` speichern, Texte getrimmt – eigene
       `toRow`-Funktion wie in `tank.ts`.
4. [ ] **Hook** `src/hooks/useCorals.ts` – Liste, Status (`loading` / `success` / `error`), Fehlermeldung, `reload()`.
       Statusnamen und Abbruch-Flag wie in `useTanks`.

## Fertig, wenn

- [ ] Kein Import von `@supabase/*` außerhalb von `src/services/` (die ESLint-Regel meldet nichts)
